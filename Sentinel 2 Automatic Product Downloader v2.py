...
Config_save_location = (
    "D:/Documents/Programming/hello/"
)  # Save location of config file.
Config = Config_save_location + "config.ini"

parser = ConfigParser()
parser.read(Config)


Username = parser.get("settings", "Username")  # Importing variable from config file
Password = parser.get("settings", "Password")
footprintName = parser.get("settings", "footprintName")
geoJsonFolder = parser.get("settings", "geoJsonFolder")

start_date = parser.get("settings", "start_date")
end_date = parser.get("settings", "end_date")
download_folder = parser.get("settings", "download_folder")
frequency = int(parser.get("settings", "frequency"))
productLevel = parser.get("settings", "productLevel")
productSize = int(parser.get("settings", "productSize"))


def download_products(
    starting_date, ending_date
):  # Download products, given start and end time and desired search area.
    ...
    )  # set cloud coverage percental

    products_df = api.to_dataframe(
        products
    )  # create pandas data frame of availible products
    ...
    )  # Get the length of the Pandas data frame that has all the products in it.
    ...
    if not os.path.exists(
        sensingDateSaveLocation
    ):  # check to see if save folder exists and creates it if not.
    ...
    for i in range(lengthDF):  # Search through all products in data frame
        ...
        fileSize = product[
            "size"
        ]  # Used to get size, small files only contain small areas of data and are undeisrable, see if statement later, want file sizes about 400 mb.
        ...
        if (
            len(str(tile_id)) == 8
        ):  # Only look for products that have 5 charecters in them, tiles only have 5.
            if (
                tile_id not in tileids_and_uuids
            ):  # see if that tile is allready in the dictionary.
                if LcLevelRegex.search(title) and fileSizeInteger > productSize:
                    tileids_and_uuids[
                        tile_id
                    ] = uuid  # wright that tileid and uuid to dictionary.
    ...
    metaDataFile = open(
        metaDataLocation, "w"
    )  # Creates metadata .csv file and deletes any old one.
    metaDataFile.close()
    ...
    for key, value in tileids_and_uuids.items():
        ...
        get_metadata = api.get_product_odata(
            value, full=True
        )  # Get Metadata of file and writes it to file.
    ...
    for file in os.listdir(root):  # extract files
        if file.endswith(".zip"):  # find file ending with .zip
            ...
            file_location = os.path.join(
                root, file
            )  # join the folder location with the zip file name
            print(file_location)
...
def geojsonLocation(geoJson_name, geoJon_location):
    ...
    footprintLocation = (
        geoJon_location + geoJson_name + ".geojson"
...
def cropped_mosaic(
    downloadFileLocation, footprintLocation
):  # For creating a mosaic of each band and cropping to the desired area from geojson file.

    geojsonFile = pygeoj.load(
        filepath=footprintLocation
    )  # load the bounds (edge) of geojson file
    ...
    u = utm.from_latlon(
        bottom, left
    )  # convert lat and long bounds to UTM bounds for files.
    left = u[0]
    bottom = u[1]
    ...
    for timePeriod in projectFolder:
        ...
        BandsFolder = (
            path + "/Cropped_Image_Bands"
        )  # location of folder to save images to

        if not os.path.exists(
            BandsFolder
        ):  # check to see if save folder exists and creates it if not. creates above folder if not all ready created.
        ...
        if productLevel == "L1C":
            for i in range(1, 16):
                ...
                foundFiles = list(
                    filter(fileRegex.findall, files)
                )  # find all files that have same ending, B01.jp2 etc.
                ...
                mosaic, out_trans = rasterio.merge.merge(
                    src_files_to_mosaic, bounds=bounds
                )  # Merge rasters, bounds = None or bounds

                out_meta = src.meta.copy()  # Copy the metadata

                out_meta.update(
                    {
                        "driver": "GTiff",  # Update the metadata
                ...
                with rasterio.open(
                    saveLocationName, "w", **out_meta
                ) as dest:  # Write the mosaic raster to disk
                    dest.write(mosaic)
...
)  # username and password for SCIHUB
footprintLocation, footprint = geojsonLocation(
    footprintName, geoJsonFolder
)  # get the footprint area of interest
...
d1 = pd.date_range(start_date, end_date, freq="1D").strftime(
    "%Y%m%d"
)  # creates the sensing periods given start and finished date and sensing frequency.
...
cropped_mosaic(
    downloadFileLocation, footprintLocation
)  # crop the bands into a mosaic with only the area of interest.
